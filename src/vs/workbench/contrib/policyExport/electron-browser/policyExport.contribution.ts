 ...
 * `value` callback). Carries only the owning policy `name`.
 */
			...
			await this.progressService.withProgress({
				...
				if (extensionPolicies) {
					...
					for (const [key, entry] of Object.entries(extensionPolicies)) {
						...
						if (!entry.name) {
							throw new Error(`Extension policy '${key}' is missing required 'name' field.`);
						}
						// A reference entry carries only the owning policy `name` (no catalog
						// metadata); the owner — and its `value` callback — is declared by an
						// in-code setting. Link it below instead of merging it as an owner.
						if (!hasKey(entry, { category: true }) || !entry.category) {
							const list = productReferencesByPolicyName.get(entry.name) ?? [];
							list.push(key);
							productReferencesByPolicyName.set(entry.name, list);
							referenced++;
							continue;
						}
						if (!entry.description) {
							throw new Error(`Extension policy '${key}' is missing required 'description' field.`);
						}
						policyData.policies.push({
							key,
							name: entry.name,
